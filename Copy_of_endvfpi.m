...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    pr=5000;
    Rsi=561;
    aa=0.00091*Tr-0.0125*(API);
    pb=18.2*(((Rsi/SGg)^0.83)*(10^aa)-1.4);
    pwf=[5000,4500,4200,4000,3900,3800,3400,3000,2700,2600,2400,2000,1800,1700,...
    1200,1000,500,0];
    qnew=[];
    
    
for w=1:numel(pwf)
% qomax=qo/(1-0.2*(1041/5000)-0.8*(1041/5000)^2);
j=qo/(pr-pb)+(pb/1.8)*(1-0.2*(1041/pb)-0.8*(1041/pb)^2);
qob=j*(pr-pb);
if pwf(w)>pb
    qon=j*(pr-pwf(w));
else
    qon=qob+((j*pb)/1.8)*(1-0.2*((pwf(w)/pb)^2)-0.8*(pwf(w)/pb)^2);
end
qnew=[qnew,qon];%#ok
end
figure;
